"""SLD — the vectorized single-step SLD engine over a fixed logic program (any arity).
...
picks the branch. The engine therefore exposes the two Prolog verbs that remain:
  ...
  derive     the SLD successor function — ONE backward resolution step from each goal state:
             ...
             Soft facts are committed separately by ``soft_ground`` (when a ``soft_scorer``
             is attached: each state's free variable is unified with its most likely neural
             filler) — invoked ONCE per final candidate set by the env's candidate
             generation, at the end of ``UnificationLogic``'s candidate pipeline, so
             delivered candidates leave unification ground.
...
from logic2rl.unification.sld.kb import KB, fact_contains
from logic2rl.unification.sld.resolution import resolve_facts, resolve_rules, resolve_soft_facts, standardize_vars
...
class SLD(nn.Module):
    """Single-step SLD grounding engine over a fixed program (see the module docstring).
    ...
    def __init__(
        ...
        *,
        ...
        # Soft-fact scoring function ``(states, counts) -> v* [B, G]`` for ``soft_ground``
        # — an nn.Module (e.g. a KGE SoftUnifier), so the compiled
        # step traces it. Attached by the application post-build (the scorer needs the
        # trained model); None ⇒ purely symbolic resolution.
    ...
    def soft_ground(self, states: Tensor, counts: Tensor) -> Tensor:
        """SOFT FACTS: unify each state's free variable with its most likely neural
        filler (``resolve_soft_facts``); identity without a ``soft_scorer``.

        Deliberately NOT part of ``derive``: the unary auto-advance re-derives up to
        ``max_unary_iterations`` times per env step, and grounding inside ``derive``
        would pay the scorer's fixed-[S,E] GEMM on every intermediate candidate set
        (measured 2x train slowdown on wn18rr). Instead the env's candidate generation
        (the end of ``UnificationLogic``'s candidate pipeline) calls this ONCE per final candidate set, after
        the unary refine — so delivered
        candidates still leave resolution ground, and the unary ``unification_safe``
        guard sees raw (uncommitted) successors as designed."""
        if self.soft_scorer is None:
            return states
        return resolve_soft_facts(states, counts, self.soft_scorer,
