...
and overrides ``derive`` to ground each free variable with a REAL KB fact
(:func:`enumerate_groundings`) *inside* the resolution, instead of a neural filler. With an
app filler attached at the seam it leaves the residual (no-real-fact) vars open for that filler;
without one they die. Configure the enumerate width ``K``, the compaction width ``S``
(= q_kge_joint_s) and the exactness ``stats`` buffer once via :meth:`set_enumerate`.
...
def enumerate_groundings(rule_goals: Tensor, rule_success: Tensor, sub_rule_idx: Tensor, fact_index,
    ...
    (no free var) passes through as one child. With ``leave_open`` (a seam filler is attached) a body
    with NO real binding emits ONE child with its var left open (for the filler); without it that
    body dies. Fixed shapes (static ``S``, ``K``, ``Y_r``) → CUDA-graph safe. ≤1-var / ≤2-atom case;
    general V-var / M-atom enumerate is the next extension."""
...
class Enumerate(BaseEngine):
    ...
    ``derive`` with the pbc-style pipeline: ``resolve_rules → enumerate_groundings (all
    real-fact groundings of each body, branching) → pack into G → prune → standardize``
    (rule→enumerate, NO parallel ``resolve_facts`` — the enumerate *is* the fact resolution).
    Without a seam filler only grounded children are emitted; with one, a body's residual
    (no-real) var is left open for the filler. Configure ``K``/``S``/``stats`` via
    :meth:`set_enumerate`."""
    ...
    def derive(self, current_states: Tensor, next_var_indices: Tensor,
        ...
        ground_goals, ground_success, ground_rid = enumerate_groundings(
            ...
            K, Y_r, S, leave_open=self.candidate_filler is not None, stats=stats)
